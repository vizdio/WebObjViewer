...
import {
    OBJECT_GENERATORS,
    type ObjectGeneratorId,
} from "./renderer/generators";
...
async function loadTextureData(file: File): Promise<TextureData> {
    ...
    try {
        ...
        if (context === null) {
            throw new Error(
                `Unable to decode texture pixels for ${file.name}.`
            );
        }
...
function App() {
    ...
    const sceneRef = useRef(
        new SceneController(
            parseObjMesh(dodecahedronObj, "Dodecahedron", "#ffffff")
        )
    );
    ...
    const [extraLights, setExtraLights] =
        useState<PointLightSettings[]>(INITIAL_EXTRA_LIGHTS);
    const [objectSettings, setObjectSettings] = useState(
        INITIAL_OBJECT_SETTINGS
    );
    ...
    const [selectedGenerator, setSelectedGenerator] =
        useState<ObjectGeneratorId>("sphere");
    const [qualitySectionCollapsed, setQualitySectionCollapsed] =
        useState(true);
    const [lightingSectionCollapsed, setLightingSectionCollapsed] =
        useState(true);
    const [transformSectionCollapsed, setTransformSectionCollapsed] =
        useState(true);
    const [materialSectionCollapsed, setMaterialSectionCollapsed] =
        useState(true);
    const [generateObjectSectionCollapsed, setGenerateObjectSectionCollapsed] =
        useState(true);
    ...
    useEffect(() => {
        ...
        const loop = (timestamp: number) => {
            ...
            if (demoModeRef.current) {
                currentObjectSettings.rotationX =
                    (currentObjectSettings.rotationX + elapsed * 0.02) % 360;
                currentObjectSettings.rotationY =
                    (currentObjectSettings.rotationY + elapsed * 0.03) % 360;
                currentObjectSettings.rotationZ =
                    (currentObjectSettings.rotationZ + elapsed * 0.015) % 360;
            }
            ...
            sceneRef.current.setLightMarkerVisible(
                currentObjectSettings.showLightMarker &&
                    currentPrimaryLight.showMarker
            );
            ...
            renderer.render({
                ...
                lights: allLights.map((light) => ({
                    position: [
                        light.x,
                        light.y,
                        Math.min(light.z, MAX_LIGHT_Z),
                    ] as [number, number, number],
                    color: light.color,
                    intensity: light.brightness,
                })),
                shadowQuality: shadowsEnabledRef.current
                    ? shadowQualityRef.current
                    : 0,
                smoothShading: smoothingEnabledRef.current,
                smoothingAngleThresholdDegrees:
                    smoothingAngleThresholdRef.current,
            ...
            if (timestamp - fpsWindowStart >= 1000) {
                const measuredFps = Math.round(
                    (frameCounter * 1000) / (timestamp - fpsWindowStart)
                );
    ...
    const handleLightChange =
        (index: number, key: keyof PointLightSettings) =>
        (event: React.ChangeEvent<HTMLInputElement>) => {
            const value = Number.parseFloat(event.target.value);
            updateLight(
                index,
                key,
                key === "z" ? Math.min(value, MAX_LIGHT_Z) : value
            );
        };
    ...
    const handleRendererModeChange = (
        event: React.ChangeEvent<HTMLSelectElement>
    ) => {
        setRendererMode(event.target.value as RendererMode);
    };

    const handleShadowQualityChange = (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
    ...
    const handleSmoothingEnabledChange = (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
        setSmoothingEnabled(event.target.checked);
    };

    const handleSmoothingAngleThresholdChange = (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
    ...
    const handleBumpIntensityChange = (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
    ...
    const handleGenerateObject = () => {
        const generator = OBJECT_GENERATORS.find(
            (option) => option.id === selectedGenerator
        );
    ...
    const handleObjFileLoad = async (
        ...
        try {
            ...
            if (objFile === undefined) {
                throw new Error(
                    "Please select an OBJ file. You can optionally include MTL and texture image files too."
                );
            }

            const objText = await objFile.text();
            const referencedLibraries = new Set(
                parseObjMaterialLibraries(objText).map((libraryName) =>
                    normalizeResourceName(libraryName)
                )
            );
            ...
            const matchedMtlFiles =
                referencedLibraries.size > 0
                    ? candidateMtlFiles.filter((file) =>
                          referencedLibraries.has(
                              normalizeResourceName(file.name)
                          )
                      )
                    : candidateMtlFiles;

            const mtlFiles =
                matchedMtlFiles.length > 0
                    ? matchedMtlFiles
                    : candidateMtlFiles;
    ...
    const removeLight = (index: number) => {
        ...
        setExtraLights((current) =>
            current.filter((_, lightIndex) => lightIndex !== index - 1)
        );
    };

    const formatLightValue = (value: number) => value.toFixed(2);
    const toggleQualitySection = () =>
        setQualitySectionCollapsed((current) => !current);
    const toggleLightingSection = () =>
        setLightingSectionCollapsed((current) => !current);
    const toggleTransformSection = () =>
        setTransformSectionCollapsed((current) => !current);
    const toggleMaterialSection = () =>
        setMaterialSectionCollapsed((current) => !current);
    const toggleGenerateObjectSection = () =>
        setGenerateObjectSectionCollapsed((current) => !current);
            ...
            <aside className="controls-panel" aria-label="Renderer controls">
                <div className="controls-scroll">
                    ...
                    <section
                        ...
                        aria-label="Object generation"
                    >
                        <div className="light-controls__header">
                            <button
                                type="button"
                                className="section-toggle"
                                onClick={toggleGenerateObjectSection}
                                aria-expanded={!generateObjectSectionCollapsed}
                            >
                                <span className="section-toggle__icon">
                                    {generateObjectSectionCollapsed ? "+" : "-"}
                                </span>
                                ...
                                <div className="light-grid">
                                    ...
                                    <label className="slider-field slider-field--wide slider-field--compact">
                                        ...
                                        <input
                                            ...
                                            onChange={
                                                handleSmoothingAngleThresholdChange
                                            }
                    ...
                    >
                        <div className="light-controls__header">
                            ...
                            <button
                                ...
                                onClick={addLight}
                                disabled={
                                    1 + extraLights.length >= MAX_POINT_LIGHTS
                                }
                            >
                                Add light
                            </button>
                                ...
                                <div className="light-grid">
                                    ...
                                    <label className="slider-field slider-field--wide slider-field--compact color-picker-field">
                                        <span>
                                            Light color{" "}
                                            <strong>
                                                {primaryLight.color.toUpperCase()}
                                            </strong>
                                        </span>
                                        <input
                                            type="color"
                                            value={primaryLight.color}
                                    ...
                                    <label className="toggle-field slider-field--wide slider-field--compact">
                                        ...
                                        <input
                                            type="checkbox"
                                            checked={
                                                objectSettings.showLightMarker &&
                                                primaryLight.showMarker
                                            }
                                ...
                                {extraLights.length > 0 && (
                                    <div className="extra-light-list">
                                        {extraLights.map((light, index) => (
                                            <section
                                                className="extra-light-card"
                                                key={light.id}
                                                aria-label={`Extra light ${index + 1}`}
                                            >
                                                <div className="light-controls__header">
                                                    <div>
                                                        <p className="eyebrow">
                    ...
                    <section
                        className="light-controls"
                        aria-label="Object transform controls"
                    >
                        <div className="light-controls__header">
                            <button
                                type="button"
                    ...
                    >
                        <div className="light-controls__header">
                            <button
                                ...
                                aria-expanded={!materialSectionCollapsed}
                            >
                                <span className="section-toggle__icon">
