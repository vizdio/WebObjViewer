...
export function parseObjMesh(
        ...
        if (type === "f") {
            ...
            for (const token of tokens.slice(1)) {
                ...
                faceVertexIndices.push(
                    parseVertexIndex(vertexToken ?? "", vertices.length)
                );
                if (textureToken !== undefined && textureToken.length > 0) {
                    faceTextureIndices.push(
                        parseTextureIndex(textureToken, textureCoords.length)
                    );
                }
            }
            ...
            if (faceVertexIndices.length >= 3) {
                ...
                const texture =
                    texturePath !== undefined
                        ? textures[normalizeResourceName(texturePath)]
                        : undefined;
                ...
                const bumpMap =
                    bumpMapPath !== undefined
                        ? textures[normalizeResourceName(bumpMapPath)]
                        : undefined;
                ...
                if (
                    ...
                    const uvTriangles =
                        triangulateFaceWithIndices(faceTextureIndices);
    ...
    return {
        ...
        color,
        triangleColors:
            triangleColors.length === triangles.length
                ? triangleColors
                : undefined,
        triangleTextureCoords:
            triangleTextureCoords.length === triangles.length
                ? triangleTextureCoords
                : undefined,
...
export function parseMtlMaterials(
    text: string
): Record<string, ParsedMtlMaterial> {
    ...
    for (const rawLine of lines) {
        ...
        if (type === "newmtl") {
            ...
            if (
                ...
                materials[activeMaterial] === undefined
            ) {
