...
function buildTriangleSamples(objects: SceneObject[]): TriangleSample[] {
    ...
    objects.forEach((object, objectIndex) => {
        ...
        const transformedVertices = object.mesh.vertices.map((vertex) =>
            transformPoint(modelMatrix, vertex)
        );

        object.mesh.triangles.forEach(
            ([indexA, indexB, indexC], triangleIndex) => {
...
function resolveOccluderDepth(
    ...
    const sampleCorner = (thetaIndex: number, phiIndex: number): number => {
        ...
        return Math.abs(sampleDistance - nearest) <= depthBias
            ? secondary
            : nearest;
    };
...
function evaluatePointVisibility(
    ...
    return clamp(
        1 - delta / Math.max(0.0001, dynamicBias * 2),
        minimumVisibility,
        1
    );
}
...
export function computePointLightShadowVisibility(
    ...
    const visibilityByObject = objects.map((object) => ({
        flat: new Array(object.mesh.triangles.length)
            .fill(0)
            .map(() => new Array(MAX_POINT_LIGHTS).fill(1)),
        perVertex: new Array(object.mesh.triangles.length)
            .fill(0)
            .map(
                () =>
                    [
                        new Array(MAX_POINT_LIGHTS).fill(1),
                        new Array(MAX_POINT_LIGHTS).fill(1),
                        new Array(MAX_POINT_LIGHTS).fill(1),
    ...
    for (let lightIndex = 0; lightIndex < lightCount; lightIndex += 1) {
        ...
        for (const sample of samples) {
            for (const point of [
                ...
                const [thetaPos, phiPos] = directionToUv(
                    vectorToTriangle,
                    thetaBins,
                    phiBins
                );
        ...
        for (const sample of samples) {
            ...
            objectVisibility.flat[sample.triangleIndex][lightIndex] =
                bestVisibility;
            if (smoothShadows) {
                objectVisibility.perVertex[sample.triangleIndex][0][
                    lightIndex
                ] = vertexVisibility[0];
                objectVisibility.perVertex[sample.triangleIndex][1][
                    lightIndex
                ] = vertexVisibility[1];
                objectVisibility.perVertex[sample.triangleIndex][2][
                    lightIndex
                ] = vertexVisibility[2];
            }
        }
