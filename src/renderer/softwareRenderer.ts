...
function sampleTexturePixel(
    ...
    const x = Math.max(
        0,
        Math.min(texture.width - 1, Math.floor(wrappedU * texture.width))
    );
    const y = Math.max(
        0,
        Math.min(
            texture.height - 1,
            Math.floor((1 - wrappedV) * texture.height)
        )
    );
...
export class SoftwareRenderer {
    ...
    render(scene: RenderScene): void {
        ...
        const cameraMatrix = createLookAtMat4(
    ...
    private drawObject(
        ...
        const cacheKey = buildObjectCacheKey(
            mesh,
            transform,
        ...
        const { transformedVertices, projectedVertices, smoothNormals } =
            cacheEntry;
        ...
        for (
            ...
            const faceNormalWorld = normalizeVec3([
                edgeABWorld[1] * edgeACWorld[2] -
                    edgeABWorld[2] * edgeACWorld[1],
                edgeABWorld[2] * edgeACWorld[0] -
                    edgeABWorld[0] * edgeACWorld[2],
                edgeABWorld[0] * edgeACWorld[1] -
                    edgeABWorld[1] * edgeACWorld[0],
            ]);

            let normalWorld = faceNormalWorld;
            if (smoothNormals !== null) {
                const [normalA, normalB, normalC] =
                    smoothNormals[triangleIndex] ?? [];
                if (
                    normalA !== undefined &&
                    normalB !== undefined &&
                    normalC !== undefined
            ...
            const triangleUv =
                mesh.triangleTextureCoords?.[triangleIndex] ?? null;
            const activeBumpMap = mesh.bumpMapData;
            if (
                bumpIntensity > 0 &&
                triangleUv !== null &&
                activeBumpMap !== undefined
            ) {
                const centroidU =
                    (triangleUv[0][0] + triangleUv[1][0] + triangleUv[2][0]) /
                    3;
                const centroidV =
                    (triangleUv[0][1] + triangleUv[1][1] + triangleUv[2][1]) /
                    3;
                ...
                const sampleHeight = (u: number, v: number): number => {
                    const x = Math.max(
                        0,
                        Math.min(
                            activeBumpMap.width - 1,
                            Math.floor(u * activeBumpMap.width)
                        )
                    );
                    const y = Math.max(
                        0,
                        Math.min(
                            activeBumpMap.height - 1,
                            Math.floor((1 - v) * activeBumpMap.height)
                        )
                    );
                ...
                if (
                    Math.abs(tangent[0]) < 0.001 &&
                    Math.abs(tangent[1]) < 0.001
                ) {
                ...
                const bumpNormal = [
                    normalWorld[0] +
                        tangent[0] *
                            (heightCenter - heightRight) *
                            bumpIntensity +
                        bitangent[0] *
                            (heightCenter - heightUp) *
                            bumpIntensity,
                    normalWorld[1] +
                        tangent[1] *
                            (heightCenter - heightRight) *
            ...
            for (
                ...
                const lambert = Math.max(
                    0,
                    dotVec3(normalWorld, objectLightDirection)
                );
                const shadow = smoothShading
                    ? ((shadowVisibility.perVertex[triangleIndex]?.[0]?.[
                          lightIndex
                      ] ?? 1) +
                          (shadowVisibility.perVertex[triangleIndex]?.[1]?.[
                              lightIndex
                          ] ?? 1) +
                          (shadowVisibility.perVertex[triangleIndex]?.[2]?.[
                              lightIndex
                          ] ?? 1)) /
                      3
                ...
                if (specularWeight > 0 && lambert > 0) {
                    ...
                    const specularDot = Math.max(
                        0,
                        dotVec3(normalWorld, halfVector)
                    );
                    specularLighting =
                        Math.pow(specularDot, specularExponent) *
                        specularWeight *
                        attenuation *
                        light.intensity *
                        shadow;
                ...
                lightMultiplierRed +=
                    light.red * (diffuseLighting + specularLighting);
                lightMultiplierGreen +=
                    light.green * (diffuseLighting + specularLighting);
                lightMultiplierBlue +=
                    light.blue * (diffuseLighting + specularLighting);
            }
    ...
    private fillTexturedTriangle(
        ...
        const minX = Math.max(
            0,
            Math.floor(Math.min(vertexA.x, vertexB.x, vertexC.x))
        );
        const maxX = Math.min(
            this.width - 1,
            Math.ceil(Math.max(vertexA.x, vertexB.x, vertexC.x))
        );
        const minY = Math.max(
            0,
            Math.floor(Math.min(vertexA.y, vertexB.y, vertexC.y))
        );
        const maxY = Math.min(
            this.height - 1,
            Math.ceil(Math.max(vertexA.y, vertexB.y, vertexC.y))
        );
        const triangleArea = edgeFunction(
            vertexA.x,
            vertexA.y,
            vertexB.x,
            vertexB.y,
        ...
        for (let y = minY; y <= maxY; y += 1) {
            ...
            for (let x = minX; x <= maxX; x += 1) {
                if (
                    ...
                    const depth =
                        vertexA.depth * normalizedA +
                        vertexB.depth * normalizedB +
                        vertexC.depth * normalizedC;
                    ...
                    if (depth < this.depthBuffer[bufferIndex]) {
                        ...
                        const shadedGreen = Math.min(
                            255,
                            baseGreen * lightGreen
                        );
