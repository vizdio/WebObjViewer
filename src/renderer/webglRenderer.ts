...
function createProgram(
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        const message =
            gl.getProgramInfoLog(program) ?? "Unknown program error.";
...
function createPerspectiveMat4(
    fieldOfView: number,
    aspectRatio: number,
    near: number,
    far: number
...
function buildFaceVertices(
    ...
    const transformedVertices = mesh.vertices.map((vertex) =>
        transformPoint(modelMatrix, vertex)
    );
    ...
    for (
        ...
        const triangleVisibility = shadowVisibility.flat[triangleIndex] ?? [
            1, 1, 1, 1,
        ];
...
export class WebGLRenderer {
    ...
    private useTextureArray: Float32Array<ArrayBufferLike> = new Float32Array(
        0
    );

    private emissiveArray: Float32Array<ArrayBufferLike> = new Float32Array(0);

    private objectCentersArray: Float32Array<ArrayBufferLike> =
        new Float32Array(0);

    private twoSidedArray: Float32Array<ArrayBufferLike> = new Float32Array(0);

    private shadowMasksArray: Float32Array<ArrayBufferLike> = new Float32Array(
        0
    );
    ...
    `;
        ...
        this.uLightIntensitiesLocation =
            this.requireUniform("uLightIntensities");
    ...
    resize(
        ...
        const pixelWidth = Math.max(
            1,
            Math.floor(cssWidth * devicePixelRatio * normalizedScale)
        );
        const pixelHeight = Math.max(
            1,
            Math.floor(cssHeight * devicePixelRatio * normalizedScale)
        );
    ...
    render(scene: RenderScene): void {
        ...
        const viewMatrix = createLookAtMat4(
            scene.cameraPosition,
            scene.cameraTarget,
            scene.cameraUp
        );
        const aspectRatio = this.currentWidth / this.currentHeight;
        const projectionMatrix = createPerspectiveMat4(
            scene.fieldOfView,
            aspectRatio,
            0.1,
            100
        );
        const background = hexToRgb(scene.background).map(
            (value) => value / 255
        ...
        for (let index = 0; index < lightCount; index += 1) {
            ...
            const [red, green, blue] = hexToRgb(light.color).map(
                (value) => value / 255
            ) as [number, number, number];
        ...
        gl.uniform1fv(
            this.uLightIntensitiesLocation,
            this.lightIntensitiesArray
        );
        gl.uniform1f(this.uAmbientLocation, 0);
        gl.uniform1f(this.uDiffuseLocation, 0.95);
        gl.uniform1f(
            this.uSheenLocation,
            Math.max(0, Math.min(1, scene.sheen))
        );
        gl.uniform1f(
            this.uBumpIntensityLocation,
            Math.max(0, Math.min(1, scene.bumpIntensity))
        );
        ...
        const shadowKey = buildShadowCacheKey(
            scene.objects,
            scene.lights,
            scene.shadowQuality,
            scene.smoothShading
        ...
        if (
            shadowKey !== this.cachedShadowKey ||
            shadowVisibilityByObject === null
        ) {
        ...
        if (vertexKey !== this.cachedVertexKey) {
            ...
            for (
                let objectIndex = 0;
                objectIndex < scene.objects.length;
                objectIndex += 1
            ) {
                const object = scene.objects[objectIndex];
                if (
                    activeTextureData === undefined &&
                    object.mesh.textureData !== undefined
                ) {
                    activeTextureData = object.mesh.textureData;
                }
                if (
                    activeBumpMapData === undefined &&
                    object.mesh.bumpMapData !== undefined
                ) {
            ...
            this.positionsArray = this.ensureArrayCapacity(
                this.positionsArray,
                vCount * 3
            );
            this.normalsArray = this.ensureArrayCapacity(
                this.normalsArray,
                vCount * 3
            );
            this.colorsArray = this.ensureArrayCapacity(
                this.colorsArray,
                vCount * 3
            );
            this.uvArray = this.ensureArrayCapacity(this.uvArray, vCount * 2);
            this.useTextureArray = this.ensureArrayCapacity(
                this.useTextureArray,
                vCount
            );
            this.emissiveArray = this.ensureArrayCapacity(
                this.emissiveArray,
                vCount
            );
            this.objectCentersArray = this.ensureArrayCapacity(
                this.objectCentersArray,
                vCount * 3
            );
            this.twoSidedArray = this.ensureArrayCapacity(
                this.twoSidedArray,
                vCount
            );
            this.shadowMasksArray = this.ensureArrayCapacity(
                this.shadowMasksArray,
                vCount * 4
            );
            ...
            this.uploadArray(
                this.positionBuffer,
                this.positionsArray.subarray(0, vCount * 3)
            );
            this.uploadArray(
                this.normalBuffer,
                this.normalsArray.subarray(0, vCount * 3)
            );
            this.uploadArray(
                this.colorBuffer,
                this.colorsArray.subarray(0, vCount * 3)
            );
            this.uploadArray(
                this.uvBuffer,
                this.uvArray.subarray(0, vCount * 2)
            );
            this.uploadArray(
                this.useTextureBuffer,
                this.useTextureArray.subarray(0, vCount)
            );
            this.uploadArray(
                this.emissiveBuffer,
                this.emissiveArray.subarray(0, vCount)
            );
            this.uploadArray(
                this.objectCenterBuffer,
                this.objectCentersArray.subarray(0, vCount * 3)
            );
            this.uploadArray(
                this.twoSidedBuffer,
                this.twoSidedArray.subarray(0, vCount)
            );
            this.uploadArray(
                this.shadowMaskBuffer,
                this.shadowMasksArray.subarray(0, vCount * 4)
            );
    ...
    private ensureArrayCapacity(buffer: Float32Array, requiredSize: number) {
        ...
        return new Float32Array(
            Math.max(requiredSize, buffer.length * 2, 1024)
        );
    }
